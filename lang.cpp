...
std::unordered_map<string, Function> Interpreter::functions;
...
void call_function(const Token& token, Enviornment& env)
{
    ...
    auto found = Interpreter::functions.find(token.string_var);
    if(found == Interpreter::functions.end()) {
...
void assignment_operation(Enviornment& env)
{
    ...
    Variable& stored_var = Interpreter::get_variable(var.string_var);
...
void Interpreter::init_functions()
{
    Function f;
    f.ptr =  &print;
    f.num_arguments = -1;
    functions.insert({"print", f});
    f.ptr = &my_rand;
    f.num_arguments = 2;
    functions.insert({"rand", f});
};
